...
/* 페이지 폴트 핸들러. 가상 메모리를 구현하기 위해 채워야 할 골격입니다.
   ...
   F의 error_code 멤버에 있습니다. 여기 예제 코드는 그 정보를
   파싱하는 방법을 보여줍니다. 이 두 가지에 대한 더 자세한 정보는
   [IA32-v3a] 섹션 5.15 "Exception and Interrupt Reference"의
   "Interrupt 14--Page Fault Exception (#PF)" 설명에서 찾을 수 있습니다. */
static void page_fault(struct intr_frame *f)
{
    bool not_present; /* True: 페이지가 존재하지 않음, false: 읽기 전용 페이지에 쓰기 시도 */
    bool write;       /* True: 쓰기 접근, false: 읽기 접근 */
    bool user;        /* True: 사용자 모드 접근, false: 커널 모드 접근 */
    void *fault_addr; /* 폴트 주소 */

    /* 폴트를 발생시킨 가상 주소를 가져옵니다.
       이 주소는 코드를 가리킬 수도 있고 데이터를 가리킬 수도 있습니다.
       반드시 폴트를 발생시킨 명령어의 주소는 아닙니다 (그건 f->rip입니다). */

    fault_addr = (void *)rcr2();

    /* 인터럽트를 다시 켭니다 (CR2를 변경되기 전에 읽을 수 있도록
       인터럽트를 끈 것입니다). */
    intr_enable();
    ...
    if (user)
    {  // 프로잭트 2에서 고의로 페이지 폴트를 발생 시켰을때의 처리
        thread_current()->exit_status = -1;
        thread_exit();
    }

    /* 폴트가 실제 폴트인 경우, 정보를 출력하고 종료합니다. */
