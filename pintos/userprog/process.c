...
{
    printf("[PROCESS_CREATE_INITD] Called with file_name='%s'\n", file_name);
...
static void initd(void* f_name)
{
    ...
    printf("[INITD] About to call process_init()\n");
    process_init();
    printf("[INITD] process_init() completed\n");

    printf("[INITD] About to call process_exec('%s')\n", (char*)f_name);
    int result = process_exec(f_name);
    printf("[INITD] process_exec returned: %d\n", result);

    if (result < 0) PANIC("Fail to launch initd\n");
    NOT_REACHED();
}
...
int process_exec(void* f_name)
{
    ...
    /* 스레드 구조체에 있는 intr_frame을 사용할 수 없습니다.
     * 현재 스레드가 재스케줄링될 때, 실행 정보가 해당 멤버에 저장되기 때문입니다. */
...
static void process_cleanup(void)
{
    ...
    if (pml4 != NULL)
    {
         ...
         * cur->pagedir를 NULL로 설정해야 합니다. 그래야 타이머 인터럽트가
         * 프로세스 페이지 디렉토리로 다시 전환할 수 없습니다.
         * 프로세스의 페이지 디렉토리를 파괴하기 전에 베이스 페이지 디렉토리를
         * 활성화해야 합니다. 그렇지 않으면 활성 페이지 디렉토리가
         * 해제된(그리고 지워진) 것이 됩니다. */
        curr->pml4 = NULL;
        pml4_activate(NULL);
        pml4_destroy(pml4);
