...
void disk_write(struct disk* d, disk_sector_t sec_no, const void* buffer)
{
    ...
    lock_acquire(&c->lock);
    select_sector(d, sec_no);
    issue_pio_command(c, CMD_WRITE_SECTOR_RETRY);  // 디스크에 쓰기 명령
    if (!wait_while_busy(d))
        PANIC("%s: disk write failed, sector=%" PRDSNu, d->name, sec_no);  // 여기서 30초 대기!
    output_sector(c, buffer);                                              // 실제 데이터 전송
    sema_down(&c->completion_wait);                                        // 인터럽트 대기
    d->write_cnt++;
    lock_release(&c->lock);
}
