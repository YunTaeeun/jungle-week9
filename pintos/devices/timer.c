...
static void timer_interrupt(struct intr_frame *args UNUSED)
{
	...
	while (!list_empty(&sleep_list)) {
		...
		if (t->wakeup_tick <= ticks) {
			...
			thread_unblock(t); // 해당 스레드를 블록 해제 (ready_list에 삽입됨)

			/* [수정] 여기서 직접 비교하지 않고 루프 종료 후 일괄 처리합니다. */
	...
	/* [수정] 수동 yield 대신 thread_test_preemption() 호출 */
	// if (yield_needed) {
	// 	intr_yield_on_return(); // 인터럽트 종료 시 yield
	// }

	/* 깨어난 스레드 중 현재 스레드보다 우선순위가 높은 스레드가 있다면 선점 */
