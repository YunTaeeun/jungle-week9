 ...
 * pointer is returned. */
/* 가상 주소 VADDR에 대한 페이지 테이블 엔트리의 주소를 페이지 맵 레벨 4(pml4)에서 반환합니다.
...
{
    uint64_t *pte = NULL; /* 최종적으로 반환할 페이지 테이블 엔트리 포인터 (초기값: NULL) */
    int idx = PML4(va); /* 가상 주소에서 PML4 인덱스 추출 (상위 9비트) */
    int allocated = 0; /* 이 함수에서 새 페이지를 할당했는지 추적하는 플래그 */
    if (pml4e)         /* PML4 테이블이 유효한 경우에만 진행 */
    {
        uint64_t *pdpe = (uint64_t *)pml4e[idx]; /* PML4 엔트리에서 PDPT 포인터 추출 (캐스팅) */
        if (!((uint64_t)pdpe & PTE_P)) /* PDPT 엔트리가 존재하지 않는 경우 (Present 비트가 0) */
        {
            if (create) /* 새 페이지 테이블을 생성해야 하는 경우 */
            {
                uint64_t *new_page = palloc_get_page(PAL_ZERO); /* 0으로 초기화된 새 페이지 할당 */
                if (new_page)                                   /* 페이지 할당 성공 시 */
                {
                    /* 새 페이지의 물리 주소를 가상 주소로 변환하고,
                     * 사용자 접근 가능(PTE_U), 쓰기 가능(PTE_W), 존재(PTE_P) 플래그 설정 */
                    pml4e[idx] = vtop(new_page) | PTE_U | PTE_W | PTE_P;
                    allocated = 1; /* 할당 플래그 설정 (나중에 정리용) */
                }
                else
                    return NULL; /* 페이지 할당 실패 시 NULL 반환 */
            }
            else
                return NULL; /* create가 false이고 페이지 테이블이 없으면 NULL 반환 */
        }
        /* PDPT 엔트리가 존재하거나 새로 생성된 경우,
         * 하위 레벨(pdpe_walk)로 재귀적으로 페이지 테이블 엔트리 찾기 */
        pte = pdpe_walk(ptov(PTE_ADDR(pml4e[idx])), va, create);
    }
    if (pte == NULL && allocated) /* 최종 PTE를 찾지 못했고, 이 함수에서 페이지를 할당한 경우 */
    {
        /* 할당했던 페이지를 해제하고 PML4 엔트리를 0으로 초기화 (메모리 누수 방지) */
        palloc_free_page((void *)ptov(PTE_ADDR(pml4e[idx])));
        pml4e[idx] = 0;
    }
    return pte; /* 찾은 페이지 테이블 엔트리 포인터 반환 (없으면 NULL) */
}
...
static void pgdir_destroy(uint64_t *pdp)
{
    for (unsigned i = 0; i < PGSIZE / sizeof(uint64_t *); i++)
    {
        if (pdp[i] & PTE_P)
        {
            uint64_t *pt = (uint64_t *)ptov(PTE_ADDR(pdp[i]));
            pt_destroy(pt);
        }
    }
