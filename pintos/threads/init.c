...
/* Breaks the kernel command line into words and returns them as
   ...
   커널 명령줄을 단어로 분리하여 argv 형식의 배열로 반환합니다.
   
   로더가 저장한 null로 구분된 인자들을 읽어옵니다.
   예: "run\0args-single onearg\0" 
       -> argv[0]="run", argv[1]="args-single onearg", argv[2]=NULL
   
   유저 프로그램의 명령줄 인자 파싱은 process_exec()에서 수행됩니다. */
static char **read_command_line(void)
{
 ...
 * ARGV 형식: argv[0]="run", argv[1]="프로그램명 인자들..."
 *
 * 예: ["run", "args-single onearg", NULL]
 *     -> argv[1]을 그대로 process_create_initd()에 전달
 *     -> process_exec()에서 공백 기준으로 파싱됨
 */
static void run_task(char **argv)
{
    const char *task = argv[1];  // "프로그램명 인자들..." 전체 문자열
    ...
    {
        // task 문자열을 그대로 전달
        // process_exec()에서 공백 기준으로 파싱할 것임
...
static void run_actions(char **argv)
{
    ...
    struct action
    {
        ...
        int argc;   /* # of args, including action name. / 액션 이름 포함 인자 개수 */
        void (*function)(char **argv); /* Function to execute action. / 액션 실행 함수 */
    };

    /* Table of supported actions. */
    /* 지원되는 액션 테이블 */
    static const struct action actions[] = {
        {"run", 2, run_task},  // "run" 액션은 최소 2개 인자 필요 (run + 프로그램명)
#ifdef FILESYS
