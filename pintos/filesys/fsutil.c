...
void fsutil_put(char **argv)
{
	...
	while (size > 0) {
		...
		disk_read(src, sector++, buffer);
		int written = file_write(dst, buffer, chunk_size);
		// printf("[FSUTIL_PUT] file_write: expected %d, got %d\n", chunk_size, written);
	...
	/* Finish up. */
	// printf("[FSUTIL_PUT] Closing file %s\n", file_name);
	file_close(dst);
	free(buffer);
	// printf("[FSUTIL_PUT] Done with %s\n", file_name);
}
