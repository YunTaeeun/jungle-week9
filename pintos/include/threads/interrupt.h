...
struct gp_registers
{
    ...
    /*x86-64의 함수 호출 규약에서 “두 번째 인자”를 RSI에 넣도록 정해져 있기 때문에 스레드 생성에서
     * 인자를 여기에 넣음음*/
...
/* 인터럽트가 발생했을 때 CPU의 상태를 저장하는 구조체
  인터럽트 처리 후 원래 작업으로 돌아가기 위해 필요한 모든 레지스터 값을 보관*/
struct intr_frame
{
    ...
    uint64_t error_code; /* 오류 코드 (필요 시 CPU가 제공) */
                         /* Pushed by the CPU.
                            These are the interrupted task's saved registers. */
    uintptr_t rip;       /* Instruction Pointer - 어디를 실행 중이었나? */
    uint16_t cs;         /* 코드 세그먼트 */
    uint16_t __pad5;     /* 패딩 */
    uint32_t __pad6;     /* 패딩 */
    uint64_t eflags;     /* CPU 플래그 - CPU 상태 (인터럽트 허용 여부 등)*/
...
void intr_register_ext(uint8_t vec, intr_handler_func*,
                       const char* name); /* 외부 인터럽트 핸들러 등록 */
void intr_register_int(uint8_t vec, int dpl, enum intr_level, intr_handler_func*,
                       const char* name); /* 내부 인터럽트 핸들러 등록 */
